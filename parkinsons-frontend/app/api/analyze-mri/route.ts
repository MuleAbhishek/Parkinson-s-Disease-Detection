...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("Calling MRI API with image:", image.name);

    // Create a new FormData object for the external API - ONLY with the image
    const apiFormData = new FormData();
    apiFormData.append("image", image);

    // Call the external MRI API
    const apiResponse = await fetch("http://127.0.0.1:8000/predict/mri/", {
      method: "POST",
      body: apiFormData,
    });
    ...
    const processingTime = `${(Math.random() * 3 + 4).toFixed(1)} seconds`;
